...
public class ResidencyCrudImpl extends EntityCrudImpl<Residency, String> {
	...
	public String delete() {
		Residency persistentObject = (Residency) converter.getAsObject(
				FacesContext.getCurrentInstance(), null,
				jsfService.getReqParam("itemId"));
		List<AuditableCollectedEntity> entities = residencyService
				.getResidencyAssociatedEvents(persistentObject);
		if (entities.size() > 0) {
			events = entities;
			showListing = false;
