...
public class ResidencyServiceImpl implements ResidencyService {
	...
	public ResidencyServiceImpl(GenericDao genericDao,
	 ...
	 * 
	 * @see
	 * org.openhds.service.ResidencyService#evaluateStartingResidency(org.openhds
	 * .model.Residency)
	 */
	public Residency evaluateResidency(Residency candidateResidency)
			throws ConstraintViolations {
		// Verify an Individual is associated with the Residency, otherwise the
		// Residency would not make sense
		...
		for (Residency previousResidency : res) {
			// its possible that the start residency being evaluated has already
			// been persisted
			// this case happens when the user is editing a residency
			// no need to have these checks on the same residency
			if (previousResidency.getUuid()
					.equals(candidateResidency.getUuid())
	...
	private void checkIndividualEligibleForNewResdency(Individual indiv)
			throws ConstraintViolations {
		if (hasOpenResidency(indiv)) {
			throw new ConstraintViolations(
					"The individual already has an open residency. You must close the current residency for this Individual before "
							+ "a new Residency can be created.");
		}
	}

	private void checkIndividualAssignedToResidency(Residency candidateResidency)
			throws ConstraintViolations {
		if (candidateResidency.getIndividual() == null) {
			log.debug("An Individual was not supplied for the Residency");
			throw new ConstraintViolations(
					"An Individual must be supplied to check if the Residency is valid.");
		}
	}

	private void checkStartDateConstraints(Residency candidateResidency,
			Residency previousResidency) throws ConstraintViolations {
		// 2 residencies cannot have the same start date - this would not make
		// sense
		if (previousResidency.getStartDate().compareTo(
				candidateResidency.getStartDate()) == 0) {
			log.debug("A residency already exist with this start date for the Individual");
			throw new ConstraintViolations(
					"Residency cannot have same start date as previous residency");
		}

		// determine if the start date of this residency is interleaved between
		// any other previous
		// residencies for this individual
		if (previousResidency.getStartDate().before(
				candidateResidency.getStartDate())
				&& previousResidency.getEndDate().after(
						candidateResidency.getStartDate())) {
			log.debug("Residency start date is interleaved with Residency with uuid: "
					+ previousResidency.getUuid());
			throw new ConstraintViolations(
					"Residency cannot have a start date that is interleaved with a previous residency.");
		}
	}

	private void checkEndDateConstraints(Residency candidateResidency,
			Residency previousResidency) throws ConstraintViolations {
		if (candidateResidency.getEndDate() != null
				&& previousResidency.getEndDate() != null) {
			// determine if end date of this residency is interleaved between
			// any other previous
			// residencies for this individual
			if (candidateResidency.getEndDate().compareTo(
					previousResidency.getStartDate()) > 0
					&& candidateResidency.getEndDate().compareTo(
							previousResidency.getEndDate()) <= 0) {
				log.debug("Residency end date is interleaved with Residency with uuid: "
						+ previousResidency.getUuid());
				throw new ConstraintViolations(
						"Residency cannot have an end date that is interleaved with a previous residency.");
			}

			// determine if residency overlays a previous residency
			if (candidateResidency.getStartDate().compareTo(
					previousResidency.getStartDate()) <= 0
					&& candidateResidency.getEndDate().compareTo(
							previousResidency.getEndDate()) >= 0) {
				log.debug("Residency overlays residency with uuid: "
						+ previousResidency.getUuid());
				throw new ConstraintViolations(
						"Residency cannot overlay a previous residency.");
			}
		} else if (candidateResidency.getEndDate() != null
				&& previousResidency.getEndDate() == null) {
			if (candidateResidency.getEndDate().after(
					previousResidency.getStartDate())) {
				throw new ConstraintViolations(
						"Residency cannot have an end date that greater than the current residency start date");
			}
		}
	}
	...
	public List<Individual> getIndividualsByLocation(Location location) {
		...
		while (itr.hasNext()) {
			...
			if (!indiv.getCurrentResidency().getLocation().getUuid()
					.equals(location.getUuid())) {
				itr.remove();
			}
		}
	 ...
	 * 
	 * @see
	 * org.openhds.service.ResidencyService#hasOpenResidency(org.openhds.model
	 * .Individual)
	 */
	 ...
	 * 
	 * @see
	 * org.openhds.service.ResidencyService#createResidency(org.openhds.model
	 * .Individual, org.openhds.model.Location, java.util.Calendar,
	 * org.openhds.model.EventType)
	 */
	public Residency createResidency(Individual individual, Location location,
			Calendar startDate, String startType, FieldWorker collectedBy) {
	...
	public List<Residency> getAllResidencies(Individual individual) {
		List<Residency> list = genericDao.findListByProperty(Residency.class,
	...
	public List<AuditableCollectedEntity> getResidencyAssociatedEvents(
			final Residency residency) {
		boolean isEnumerationResidency = residency.getStartType().equals(
				siteProperties.getEnumerationCode());
		...
		if (residency.getEndType() != null) {
			if (residency.getEndType().equals(siteProperties.getDeathCode())) {
				AuditableCollectedEntity ae = genericDao.findByMultiProperty(
						Death.class,
						ValuePropertyBuilder.build("deathDate",
								residency.getEndDate()),
						ValuePropertyBuilder.build("individual",
								residency.getIndividual()));
				if (ae == null) {
			...
			if (residency.getEndType().equals(
					siteProperties.getOutmigrationCode())) {
				AuditableCollectedEntity ae = genericDao.findByMultiProperty(
						OutMigration.class,
						ValuePropertyBuilder.build("recordedDate",
								residency.getEndDate()),
						ValuePropertyBuilder.build("individual",
								residency.getIndividual()));
				if (ae == null) {
		...
		if (residency.getStartType()
				.equals(siteProperties.getInmigrationCode())) {
			AuditableCollectedEntity ae = genericDao.findByMultiProperty(
					InMigration.class,
					ValuePropertyBuilder.build("recordedDate",
							residency.getStartDate()),
					ValuePropertyBuilder.build("individual",
							residency.getIndividual()));
			if (ae == null) {
		...
		if (residency.getStartType().equals(siteProperties.getBirthCode())) {
			AuditableCollectedEntity ae = genericDao.findByMultiProperty(
					...
					ValuePropertyBuilder.build("recordedDate",
							residency.getStartDate()),
					ValuePropertyBuilder.build("child1",
							residency.getIndividual()));
			if (ae == null) {
				// try the second child
				ae = genericDao.findByMultiProperty(
						PregnancyOutcome.class,
						ValuePropertyBuilder.build("recordedDate",
								residency.getStartDate()),
						ValuePropertyBuilder.build("child2",
								residency.getIndividual()));
			}
