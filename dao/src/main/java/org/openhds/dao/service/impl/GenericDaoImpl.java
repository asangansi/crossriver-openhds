...
public class GenericDaoImpl implements GenericDao {
    ...
    public <T> T findUniqueByPropertyWithOrder(Class<T> entityType, String propertyName, Object value,
        ...
        return (T) getSession().createCriteria(entityType).add(Restrictions.eq(propertyName, value))
                .add(Restrictions.eq("deleted", false)).addOrder(order).setMaxResults(1).uniqueResult();
    }
    ...
    @SuppressWarnings("unchecked")
    public <T> List<T> findListByPropertyWithOrder(Class<T> entityType, String propertyName, Object value,
    ...
    public <T> long getTotalCount(Class<T> entityType) {
        return (Long) getSession().createCriteria(entityType).setProjection(Projections.rowCount()).uniqueResult();
    }
